...
## 5. Attach a KV database to this new project

Because this is a new project, it doesn't automatically have the KV
database from before. You have two options:

**Option A — reuse your existing data.** In the new project, go to
**Storage** → **Connect Database** (rather than "Create New") → select the
KV database you already created for your original project. This links the
same underlying storage to this project too, so all your existing chords,
practice log, and leaderboard entries are still there.

**Option B — start fresh.** Go to **Storage** → **Create Database** → KV,
and attach a brand new one. This is simpler, but you'll start with an
empty log (your old data will still exist under the original project,
just not connected to this one).
...
Your data lives in Vercel KV, separate from these files, so routine code
...
- **Site loads but storage doesn't work:** double check a KV database is
  attached under Project → Storage, and that you redeployed after
  attaching it.
